...
//  var date= item.created_at;
 // var time= item.created_at;
  ...
  $("#post-list").append('<div class="post-content"><div class="post-container"><img src="'+user_img+'" style="min-width: 50px;border-radius: 50%;max-width: 40px;max-height: 50px;min-height: 40px;" class="profile-photo-md pull-left"><div class="post-detail" id="post-data"><div class="user-info"><h5><a href="" class="profile-link">'+name+'</a><p style="float:right;"><i class="icon ion-ios-calendar-outline"></i>date &nbsp; <i class="icon ion-ios-clock-outline"></i> Time </p></h5></div><div class="post-text"><p>'+post_text+' </p></div></div> <form id="myform"  method="POST"  class="form_statusinput"> <textarea class="form-control comment" id="comment" name="comment" placeholder="write a comment..." rows="2"></textarea><br> <input type="hidden" name="postid" id="postid" value=" "><input type="hidden" name="username" id="username" value="  "><input type="submit" id="post" class="btn btn-info pull-right" value="Comment"></form>  <div class="clearfix"></div><div class="post-comment">  test             <img src="http://localhost:8000/images/user_profile_pics/photoicon.jpg " alt="" class="img-circle" class="profile-photo-sm" style="height:40px;min-width:40px;max-width:40px;">                    <p><a href="userprofile.php?id= " class="profile-link">&nbsp;    </a>   </p> </div></div></div> ');
});
}
